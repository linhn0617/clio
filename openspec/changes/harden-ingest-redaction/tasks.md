...
- [ ] 1.1 Failing tests in `redact_test.go`: `Redact` redacts `postgres://user:pass@host/db`
  and `redis://h:p@x` but leaves `https://example.com` and `https://user@host` (no
  password) untouched.
- [ ] 1.2 Add the connection-string rule to `redactRules`; tests green.
- [ ] 1.3 Failing tests for `isSecretKey`: TRUE for `apiKey`,`API_KEY`,`token`,
...
- [ ] 1.4 Implement `isSecretKey` as a lowercase word-boundary regex (NOT substring); tests green.

## 2. redactString + redactJSON (TDD)

- [ ] 2.1 Failing tests for `redactString`: plain text with `sk-…20+` redacted by regex;
  a JSON-as-text value `{"apiKey":"secret-value-123456"}` → key value `[REDACTED:key]`;
  plain prose unchanged; CJK text unchanged.
- [ ] 2.2 Failing tests for `redactJSON([]byte) []byte`:
...
- [ ] 2.3 Implement `redactString` (regex + JSON-aware recurse) and `redactJSON`
...
- [ ] 3.1 Failing ingest regression test in `ingest_test.go`: ingest a session whose
...
- [ ] 3.2 In `parser.go`: `raw := string(redactJSON(line))`; `content` via
  `redactString`; `TitleHint` via `redactString(text)` at both call sites
  (`asString` and `b.Text`). Run 3.1 green.
- [ ] 3.3 Existing redaction/parser/ingest tests still pass (adjust any that asserted
