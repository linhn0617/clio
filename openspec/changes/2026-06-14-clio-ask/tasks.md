...
- [ ] 1.1 Term extraction (TDD): strip bilingual stopwords / question words to
  content terms; fall back to all terms when the question is all stopwords.
- [ ] 1.2 Any-term (OR) retrieval (TDD): add an any-term FTS match builder in
...
- [ ] 1.3 Windowing (TDD): `sessions.GetWindow` returns ±N user/assistant turns
  around a hit seq (conversational turn space, not raw seq), excluding tool output
  by default.
- [ ] 1.4 Assemble (TDD): group candidates by session, merge overlapping windows,
...
- [ ] 2.1 `clio ask` command (TDD): `--project` (default all), `--since`,
...
- [ ] 2.2 `ask` MCP tool (TDD): params question/project/since/limit; structured
  JSON bundle; read-only annotation; registered in the server.

## 3. Verify

- [ ] 3.1 `go build/vet/test ./...` green; `openspec validate --strict`.
