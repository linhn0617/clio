...
- [x] 1.1 Build the registry as a static-seeded slice + helpers — `Names()`,
...
- [x] 2.1 CLI: `validateSource` + `--source` help/default (`internal/cli/common.go:26,
  31-36`) derive from the registry (TDD).
- [x] 2.2 CLI bootstrap: generalize `codexAvailable` (`internal/cli/common.go:39-48`) to
...
- [x] 2.3 MCP: one helper feeds the five tools' `mcp.Enum` values + `DefaultString`
...
- [x] 2.4 doctor: iterate registry sources for root presence/health instead of the codex
...
- [x] 2.5 TUI: row labels via `registry.Label` (`internal/tui/browse_view.go:249`,
  `internal/tui/search_view.go:229`) (TDD).
- [x] 2.6 DB: the source-filter default in `internal/db/db.go:182-191` takes the default
  source name from the registry (constants stay the values) (TDD).

## 3. Golden-test gate (behavior unchanged)

- [x] 3.1 Golden tests assert string-identity with the pre-change hardcoded values, with
...
- [x] 4.1 `go build/vet/test ./...` green (incl. `-race` and windows cross-build);
...
- [x] 4.2 Third-party (codex) review of the real diff to a clean gate — specifically that
