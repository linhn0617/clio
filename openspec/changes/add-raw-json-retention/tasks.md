...
- [ ] 1.1 Migration 0012: (a) recreate `messages_au` as `AFTER UPDATE OF content`
...
- [ ] 1.2 Tests: raw_json-only UPDATE leaves messages_fts postings unchanged
  ...
  re-ingest; **migration on a pre-0012 DB sets every existing row aborted=1, and
  a subsequent `index --full` clears validated rows to 0**

## 2. Pruning query & selection

- [ ] 2.1 sessions-layer prune: single write-transaction UPDATE with
  `raw_json<>''` guard over restorable eligible sessions (age + regular/readable
  file + under a scanned root for the recorded source + non-lagged ingest_state);
...
- [ ] 2.2 Dry-run: same selection, returns counts, writes nothing; duration
  parsed via timeutil.ParseSince, reject zero/negative/malformed
- [ ] 2.3 Read-only pending-backfill exclusion: skip (do NOT backfill)
...
- [ ] 2.4 Tests: old+restorable pruned; recent untouched; each skip reason
...
- [ ] 3.1 `clio prune-raw --older-than <dur> [--dry-run] [--vacuum] [--source]
  [--project]`: summary of pruned + skipped-by-reason + restore hint
- [ ] 3.2 `--vacuum`: check MCP lock BEFORE any mutation → non-zero + full no-op
  if held; else prune then VACUUM; without --vacuum print reclaimable + how-to
- [ ] 3.3 Command-level tests (sandboxed HOME/XDG): prune→verify blanked;
  --vacuum under held lock is a full no-op (nothing pruned); malformed duration
  rejected

## 4. Restore path & visibility

- [ ] 4.1 `clio index --full` post-run check: report + non-zero exit if any
  session still has pruned raw_json after a full run
- [ ] 4.2 Restore tests for ALL THREE sources (Claude re-parse, Codex rollout,
  Gemini replay): prune → index --full → raw_json repopulated; plus an
  unrestorable case (file made unreadable) surfaced + non-zero

## 5. show degradation

- [ ] 5.1 `--format raw`: per-session pruned note (restore hint) + non-pruned
  lines; fully-pruned session prints only the note
- [ ] 5.2 `--format json`: `raw_json: null` on pruned messages + top-level
  `raw_pruned: true`; markdown path unchanged
- [ ] 5.3 Tests for all three formats on pruned/partly-pruned/unpruned sessions

## 6. doctor & docs

- [ ] 6.1 doctor: exact pruned-message count + labeled whole-DB freelist figure;
  zero/clean when nothing pruned; test both
- [ ] 6.2 README/USAGE: `clio prune-raw` (reversibility, the restorable-not-just-
  existing rule, --vacuum lock behavior, restore via index --full)

## 7. Verification

- [ ] 7.1 `go build ./... && go vet ./... && go test ./...` green
- [ ] 7.2 Prune perf gates on a fixture, from a wal_checkpoint(TRUNCATE) baseline
  ...
  BLOB bytes
- [ ] 7.3 Every spec `#### Scenario` maps to a test
- [ ] 7.4 Fresh-context verifier + adversarial cross-model review before archive
