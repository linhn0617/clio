...
- [ ] 1.1 Failing test in `internal/db/db_test.go`: after `Open`, `ingest_state` has
  columns `head_fingerprint` and `unparsed_lines` (query `PRAGMA table_info`).
- [ ] 1.2 Add `internal/db/migrations/0005_ingest_state_cols.sql`:
...
- [ ] 2.1 Failing test in `internal/ingest/incremental_test.go` (or ingest_test): a
...
- [ ] 2.2 Add `headFingerprint` (first 512 bytes) to `FileState`; compute and persist
...
- [ ] 2.3 Failing test (eng-review D2): a row with stored `head_fingerprint = ''`
...
- [ ] 3.1 Failing/seam test in `ingest_test.go`: ingest a file with many lines via the
  ...
  complete newline.
- [ ] 3.2 Replace `readFrom`/`io.ReadAll`/`splitLines` slurp with a `bufio.Reader`
...
- [ ] 3.3 Failing tests (eng-review D3 + outside-voice #1): (a) a single line exceeding
...
- [ ] 4.1 Failing tests in `ingest_test.go` (outside-voice #2 semantics): (a) a file with
...
- [ ] 4.2 In the streaming loop, count `ParseLine` errors per pass; in `commit()` ADD the
...
- [ ] 5.1 Failing test in `ingest_test.go` using `preCommitHook` to REMOVE the source
...
- [ ] 5.2 In `commit()`, change the `os.Stat` guard so `statErr != nil` returns
...
- [ ] 6.1 Failing test in `internal/doctor/doctor_test.go`: with `unparsed_lines > 0`
  in `ingest_state`, `doctor` adds a non-zero warning check naming the count and
  suggesting `clio index --full`; zero → passing/clean.
- [ ] 6.2 Add the check to `internal/doctor/doctor.go` (sum `unparsed_lines`). Green.

## 7. Verify

- [ ] 7.1 `go test ./internal/ingest/ ./internal/db/ ./internal/doctor/ -race -count=1`
  green.
- [ ] 7.2 `go test ./... -count=1`, `go vet ./...`, `go build ./...`,
  `GOOS=windows GOARCH=amd64 go build ./...` clean; `gofmt -l` empty.
- [ ] 7.3 Self-review, then codex adversarial re-review of the diff; address findings.
