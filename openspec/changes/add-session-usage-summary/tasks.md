...
- [ ] 1.1 Migration 0011: `session_usage` (PK session_uuid+model, source/model NOT NULL with
...
- [ ] 1.2 Real-sample fixtures (from real local files, redacted): Claude session with usage
  ...
  tail-ingest gate
- [ ] 1.3 Baseline measurements per the design.md protocol (fresh DB, wal_checkpoint(TRUNCATE),
...
- [ ] 2.1 session_usage write helper with three outcomes (replace = delete session rows +
...
- [ ] 2.2 Claude: dedicated usage pass over the original source file (extract outer uuid /
...
- [ ] 2.3 Codex: extract in the currently-skipped token_count branch — latest cumulative per
...
- [ ] 2.4 Gemini: sum tokens during whole-file replay (replay with no usage deletes stale row);
  re-index idempotency test; unmapped categories (e.g. thoughts mapping decision) land in
  categories_json + usage_unmapped
- [ ] 2.5 `clio index --full` backfill verified on a pre-feature DB (empty → populated); lock
  behavior fixed: `--full` under MCP lock exits non-zero with stop-the-server message, never
  "nothing to do" success
- [ ] 2.6 usage_skipped/usage_unmapped counter semantics test: whole-file passes (Claude scan,
...
- [ ] 2.7 total_tokens semantics implemented per spec (native totals for Codex/Gemini, fixed
  derived sum for Claude; categories_json never contributes) with a cross-source fixture test

## 3. Surfaces

- [ ] 3.1 `clio usage` subcommand (--since/--project/--source/--by session|project|model):
...
- [ ] 3.2 `clio usage --quota`: staleness rendering (age line; stale when older than window OR
  resets_at in past; fixed "last-observed, not live" disclaimer)
- [ ] 3.3 `clio list` + TUI token column (placeholder when no data, not zero; stale marking
  test in both)
- [ ] 3.4 MCP activity_summary group_by="usage": session-level aggregates with source
...
- [ ] 4.1 doctor: report usage coverage (sessions with usage / total),
  usage_skipped/usage_unmapped counts, and stale-usage file count (usage_stale flags)
- [ ] 4.2 Fix stale debt comments found in review: 16MiB wording (gemini.go — abort not silent
  truncation) and claudeconfig atomic-write comment (remaining debt is backup-failure semantics)
- [ ] 4.3 README/USAGE docs: usage command, per-source non-comparability note, quota staleness
...
- [ ] 5.1 Post-checkpoint DB growth vs 1.3 baseline < 2%; doctor DB/source ratio stays below
  the 3× warning
- [ ] 5.2 Full-index wall-clock regression < 5% (interleaved, n≥10, medians)
- [ ] 5.3 Long-session tail-ingest gate: usage pass adds < 30% wall-clock, write-lock hold
...
- [ ] 5.4 Gemini single-session re-replay combined DB+WAL growth < 10%
- [ ] 5.5 Grep gate: no monetary amount stored, computed, or displayed anywhere in this change
- [ ] 5.6 Fresh-context verifier + adversarial review pass over the implementation (per repo
  convention) before release notes claim the feature
