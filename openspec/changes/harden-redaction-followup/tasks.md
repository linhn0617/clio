...
- [ ] 1.1 Failing tests in `internal/ingest/redact_test.go`: `Redact` replaces the
...
- [ ] 1.2 Add the two rules to `redactRules` in `internal/ingest/redact.go`
  (`(?i)\bBasic\s+[A-Za-z0-9+/=]{8,}` and `(?i)\b(Set-)?Cookie:\s*[^\r\n]+`). Green.

## 2. authorization / cookie secret keys (TDD)

- [ ] 2.1 Failing tests in `redact_test.go`: `isSecretKey` returns true for
...
- [ ] 2.2 Add `authorization`, `cookie`, `set cookie` to `secretKeyRe` in
  `internal/ingest/redact.go`. Green.

## 3. Ingest regression (TDD)

- [ ] 3.1 Failing ingest regression in `internal/ingest/ingest_test.go`: ingest a
  ...
  base64 / cookie value.
- [ ] 3.2 Confirm green with the rules + keys from tasks 1-2 (no parser change needed;
