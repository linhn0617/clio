...
- [ ] 1.1 Failing test in `internal/ingest/ingest_test.go`: ingest two session files,
...
- [ ] 1.2 Failing test: a source file that still exists but returns a non-`NotExist`
  stat error (simulate via a stat seam) is NOT purged.
- [ ] 1.3 Implement `PurgeMissing(ctx, projectsDir)` in `internal/ingest`: enumerate
...
- [ ] 1.4 Failing tests for the blast-radius guards (eng-review D1 + outside-voice #3/#6):
...
- [ ] 2.1 Failing test in `internal/watcher/watcher_test.go`: after a backstop tick with
  a deleted source, the session is purged from the DB.
- [ ] 2.2 Call `PurgeMissing` in the watcher backstop branch (alongside `IngestAll`) and
  in the CLI catch-up path. Green; existing watcher/CLI tests still pass.

## 3. pidAlive treats EPERM as alive (C8, TDD)

- [ ] 3.1 Failing test in `internal/lock/lock_test.go`: with the `signalProc` seam
  overridden to return `syscall.EPERM`, `pidAlive(pid)` returns true; returning `nil`
  → true; returning `os.ErrProcessDone`/ESRCH → false.
- [ ] 3.2 Add the `signalProc` package seam; change `pidAlive` to return true on `nil`
  or `errors.Is(err, syscall.EPERM)`. Green; existing lock tests pass.

## 4. Verify

- [ ] 4.1 `go test ./internal/ingest/ ./internal/watcher/ ./internal/lock/ ./internal/cli/ -race -count=1`
  green.
- [ ] 4.2 `go test ./... -count=1`, `go vet ./...`, `go build ./...`,
  `GOOS=windows GOARCH=amd64 go build ./...` clean; `gofmt -l` empty.
- [ ] 4.3 Self-review, then codex adversarial re-review of the diff; address findings.
