...
- [ ] 1.1 Bubble Tea root model (TDD): tab bar + tab switching (`Tab`/`Shift-Tab`,
  `1`–`4`), master-detail split, shared preview, status line; `Update` transitions
  tested without a real terminal.
- [ ] 1.2 `clio tui` command: open the index read-only after a startup catch-up
...
- [ ] 2.1 Search view: debounced live `search.Search`; list + selection; preview
  the hit session via `sessions.GetMessages` with the matched line marked.
- [ ] 2.2 Browse view: `sessions.ListSessions` (recent / `--project`); preview.
- [ ] 2.3 Activity view: `sessions.ActivityByKind`; selecting an entry drills to its
  sessions (`Touched`/`Ran` filter).
- [ ] 2.4 Ask view: question input → `ask.Ask`; render the grouped evidence bundle;
  preview the selected group's excerpts.

## 3. Cross-cutting

- [ ] 3.1 Queries run as `tea.Cmd` (UI never blocks); query errors show in the
