...
**預編譯（推薦）** —— 從 [最新 release](https://github.com/linhn0617/clio/releases/latest)（目前為 [**v0.6.0**](https://github.com/linhn0617/clio/releases/tag/v0.6.0)；macOS/Linux `amd64`+`arm64`、Windows `amd64`）下載你平台的檔案，放進 `PATH`，並用 `SHASUMS256.txt` 驗證。此 repo 為 private，請在登入 GitHub 的狀態下下載。

**或用 `go install`：**

```
export GOPRIVATE=github.com/linhn0617/*   # private repo：設定一次
go install github.com/linhn0617/clio/cmd/clio@v0.6.0   # 或用 @latest 取得最新
```

這會把 `clio` 放進 `$(go env GOPATH)/bin`，確認該目錄在 `PATH` 裡，且你的 git/GitHub 認證能 clone 這個 private repo。
