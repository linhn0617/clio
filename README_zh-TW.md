...
**預編譯（推薦）** —— 從 [最新 release](https://github.com/linhn0617/clio/releases/latest)（目前為 [**v0.9.0**](https://github.com/linhn0617/clio/releases/tag/v0.9.0)；macOS/Linux `amd64`+`arm64`、Windows `amd64`）下載你平台的檔案，放進 `PATH`，並用 `SHASUMS256.txt` 驗證。此 repo 為 private，請在登入 GitHub 的狀態下下載。

**或用 `go install`：**

```
export GOPRIVATE=github.com/linhn0617/*   # private repo：設定一次
go install github.com/linhn0617/clio/cmd/clio@v0.9.0   # 或用 @latest 取得最新
```
