...
**預編譯（推薦）** —— 從 [最新 release](https://github.com/linhn0617/clio/releases/latest)（目前為 [**v0.13.0**](https://github.com/linhn0617/clio/releases/tag/v0.13.0)；macOS/Linux `amd64`+`arm64`、Windows `amd64`）下載你平台的檔案，放進 `PATH`，並用 `SHASUMS256.txt` 驗證。binary 未經簽章：macOS
...
```
go install github.com/linhn0617/clio/cmd/clio@v0.13.0   # 或用 @latest 取得最新
```
...
回歸測試套件保護：以小型雙語 fixture corpus 跑在 `internal/eval`，屬於一般
`go test ./...` 的一部分。
