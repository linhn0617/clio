...
// TestIngestTitleSkipsCaveatBoilerplate: a session whose first user "message"
// is the harness-injected <local-command-caveat> banner (verbatim, with no
// real content attached) must derive its title from the next user message
// that actually carries content, not from the banner text.
func TestIngestTitleSkipsCaveatBoilerplate(t *testing.T) {
	projects := t.TempDir()
	caveat := `{"type":"user","timestamp":"2026-04-26T11:00:00Z","cwd":"/p","sessionId":"sess-cav","message":{"role":"user","content":"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>"}}`
	realQ := `{"type":"user","timestamp":"2026-04-26T11:00:01Z","sessionId":"sess-cav","message":{"role":"user","content":"幫我對這個 repo 做全盤 review"}}`
	writeSession(t, projects, "-p", "sess-cav", caveat, realQ)
	...
	var title string
	if err := d.QueryRow(`SELECT COALESCE(title,'') FROM sessions WHERE uuid='sess-cav'`).Scan(&title); err != nil {
		t.Fatal(err)
	}
	if title != "幫我對這個 repo 做全盤 review" {
		t.Fatalf("title=%q, want the real question (caveat banner must not win)", title)
	}
}

// TestIngestTitleFallsBackWhenOnlyBoilerplate: if every user message in a
// session is harness boilerplate, the title stays empty rather than showing
// a wrapper banner — existing downstream fallback behavior is preserved.
func TestIngestTitleFallsBackWhenOnlyBoilerplate(t *testing.T) {
	projects := t.TempDir()
	caveat := `{"type":"user","timestamp":"2026-04-26T11:00:00Z","cwd":"/p","sessionId":"sess-only-cav","message":{"role":"user","content":"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to.</local-command-caveat>"}}`
	notif := `{"type":"user","timestamp":"2026-04-26T11:00:01Z","sessionId":"sess-only-cav","message":{"role":"user","content":"<task-notification>\n<task-id>a1</task-id>\n<status>completed</status>\n</task-notification>"}}`
	writeSession(t, projects, "-p", "sess-only-cav", caveat, notif)
	...
	var title string
	if err := d.QueryRow(`SELECT COALESCE(title,'') FROM sessions WHERE uuid='sess-only-cav'`).Scan(&title); err != nil {
		t.Fatal(err)
	}
	if title != "" {
